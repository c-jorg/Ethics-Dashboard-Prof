...
class Answer(db.Model):
    ...
    def json(self):
        return {'id': self.id, 'assignment_id': self.assignment_id, 'form_id': self.form_id, 'key': self.key,
                'value_string': self.value_string, 'value_int': self.value_int, 'created': self.created,
    ...
    def delete_answers_by_assignment_id_and_form_id(cls, assignment_id, form_id):
        print("deleting answers for assignment id and form id in answer model " + str(
            assignment_id) + " and form id " + str(form_id))
        answers = cls.query.filter(cls.assignment_id == assignment_id, cls.form_id == form_id).all()
