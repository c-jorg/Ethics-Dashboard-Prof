...
def get_professor_classes(prof_id):
    try:
        ...
        classes = Class.get_all_classes_by_professor_id(prof_id)
        if len(classes) == 0:
            return jsonify({'message': 'No classes found for this professor'}), 404 
        classes_list = []
        #print("making class list", flush=True)
        ...
        return jsonify({'classes':classes_list, 'message':'Successfully found classes for this professor'}), 200
...
def add_class():
    ...
    try:
        ...
        return make_response(jsonify({'success': False, 'message':'Invalid input data'}), 400)
    
    # Check if class_id already exists
    if Class.get_class_by_class_code(class_code):
        return jsonify({'success': False, 'message': 'Class with that code already exists'}), 409

    Class.post_class(class_name, professor_id, class_code)
    class_id = Class.get_class_id_by_class_code(class_code)
    if not class_id:
        return make_response(jsonify({"success": False, "message":"error making class"}), 500)
    class_id = Class.get_class_id_by_class_code(class_code)
    ...
    for student_data in students:
        ...
        if not enrollment:
            ...
            try:
                ...
                #db.session.rollback()
                print("error adding student", flush=True)
                continue
...
def get_class_by_id(class_id):
    try:
        ...
        cls = Class.get_class_by_class_id(class_id)
        if not cls:
